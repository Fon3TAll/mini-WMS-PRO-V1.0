...
export default function FGGoodsReceipt() {
  ...
  const [mockReceipts, setMockReceipts] = useState<any[]>([
    { id: 'FG-IN-2605-001', so: 'PRD-LINE-A', sku: 'FG-8801', name: 'Nescafe Red Cup 380g (เนสกาแฟ)', qty: 15200, unit: 'BOX', lot: 'L2026101', exp: '2028-12-31', currentStock: 8000, safetyStock: 12000 },
    { id: 'FG-IN-2605-002', so: 'PRD-LINE-B', sku: 'FG-8802', name: 'Singha Water 600ml Pack 12', qty: 4500, unit: 'PACKS', lot: 'L2026102', exp: '2028-12-31', currentStock: 3000, safetyStock: 2500 },
  ]);

  // --- Dynamic Scan-to-Auto-Fill Parser Helper for FG ---
  const handleFormScan = (code: string) => {
    let updated = { ...formGR };
    
    if (code === 'PLT-FG-2026-809') {
      updated.lot = 'L2026-809';
      updated.sku = 'FG-8801';
      updated.name = 'Nescafe Red Cup 380g (เนสกาแฟ)';
      updated.unit = 'BOX';
      updated.currentStock = '8000';
      updated.safetyStock = '12000';
      updated.exp = '2028-12-31';
      updated.so = 'PRD-LINE-A';
      Swal.fire({
        icon: 'success',
        title: 'Auto-Filled Finished Goods Info',
        text: `สแกนรหัสพิกัดพาเลท [PLT-FG-2026-809]: พบความคืบหน้ารับ Nescafe แบรนด์หลัก เติมคุณลักษ์ผลิตภัณฑ์เรียบร้อย!`,
        timer: 1800,
        showConfirmButton: false,
        toast: true,
        position: 'top-end'
      });
    } else if (code === 'SO-BRN-30114') {
      updated.so = 'DIST-WEST-01';
      updated.lot = 'L' + new Date().getFullYear() + '-SO114';
      Swal.fire({
        icon: 'success',
        title: 'Auto-Filled Dispatch Order',
        text: `พบรหัสใบนำสินค้าออก/สาขา [SO-BRN-30114] นำเข้ารหัสสายงานขนถ่ายเรียบร้อย!`,
        timer: 1800,
        showConfirmButton: false,
        toast: true,
        position: 'top-end'
      });
    } else if (code === 'SKU-PKG-771') {
      updated.sku = 'FG-8899';
      updated.name = 'กล่องลูกฟูกเปล่าทนไฟสากล Kraft Box-XL';
      updated.unit = 'PACKS';
      updated.currentStock = '500';
      updated.safetyStock = '400';
      updated.lot = 'LOT-PKG-771';
      updated.exp = '2029-12-31';
      Swal.fire({
        icon: 'success',
        title: 'Auto-Filled Packaging Item',
        text: `สแกนพบโค้ดกล่องห่อหุ้ม [SKU-PKG-771] เขียนบันทึกโมเด็กและรายละเอียดเรียบร้อย!`,
        timer: 1800,
        showConfirmButton: false,
        toast: true,
        position: 'top-end'
      ...
      updated.lot = 'LOT-' + code;
      Swal.fire({
        icon: 'warning',
        title: 'Warehouse Location Scanned',
        text: `โค้ดนี้คือตำแหน่งคลังเก็บ [${code}] นำไปอิงเข้าในบันทึก Lot ย่อยเพื่อการสอบทวนด่วน`,
        timer: 2000,
        showConfirmButton: false,
        toast: true,
        position: 'top-end'
      ...
      updated.sku = code;
      updated.lot = 'LOT-FG-' + code.slice(-3) + '-' + Math.floor(100 + Math.random() * 900);
      Swal.fire({
        icon: 'info',
        title: 'Generic ID Scanned',
        text: `นำรหัสที่สแกน [${code}] ไปป้อนในช่องรหัสสินค้า SKU และ LOT ชั่วคราวเรียบร้อย`,
        timer: 1500,
        showConfirmButton: false,
        toast: true,
        position: 'top-end'
  ...
  const handleAddReceiptSubmit = (e: React.FormEvent) => {
    ...
    if (!formGR.sku || !formGR.name || !formGR.qty) {
      Swal.fire({
        ...
        text: 'กรุณากรอกรหัสสินค้า SKU, ชื่อ และระบุปริมาณที่เบิกสำเร็จ!'
      });
      return;
    }

    const newID = formGR.id.trim() || `FG-IN-2606-${Math.floor(100 + Math.random() * 900)}`;
    const newRecord = {
      id: newID,
      so: formGR.so || `PRD-LINE-${String.fromCharCode(65 + Math.floor(Math.random() * 3))}`,
      sku: formGR.sku,
      name: formGR.name,
      qty: parseFloat(formGR.qty) || 0,
      unit: formGR.unit,
      lot: formGR.lot || `L${new Date().getFullYear()}${Math.floor(1000 + Math.random() * 9000)}`,
      exp: formGR.exp || '2028-12-31',
      currentStock: parseFloat(formGR.currentStock) || 0,
      safetyStock: parseFloat(formGR.safetyStock) || 1000
    };
    ...
    setFormGR({
      id: '', so: '', sku: '', name: '', qty: '', unit: 'BOX', lot: '', exp: '', currentStock: '0', safetyStock: '2000'
    });

    Swal.fire({
      icon: 'success',
      title: 'ตรวจปล่อยสินค้าเสร็จสิ้น!',
      text: `สินค้าสำเร็จรูปหมายเลขการจัดเก็บ ${newID} พร้อมจัดเก็บหรือขึ้นแท็บแร็คพิมพ์แล้ว`,
      timer: 2000,
      showConfirmButton: false
    });
  };
          ...
          customHeader={
              <div className="print:hidden flex justify-between items-center bg-[#212c46] text-white p-4 sticky top-0 z-50 shadow-md cursor-move w-full rounded-t-3xl">
                  <div className="flex items-center gap-3">
                      ...
                      <div>
                          <h2 className="font-black tracking-widest text-[13px] uppercase">Print Preview: FG Tags</h2>
          ...
          <div className="flex items-center gap-5">
              ...
              <div className="text-left">
                  <h3 className="font-black text-[#212c46] uppercase tracking-tighter leading-none" style={{ fontSize: '24px' }}>
                      INBOUND <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#3f809e] to-[#b58c4f]">VERIFY FG</span>
                  </h3>
                  <div className="flex items-center gap-1.5 mt-[6px]">
                      ...
                      <p className="text-[11px] font-bold text-[#676767] uppercase tracking-[0.2em] mt-0.5 opacity-80 leading-none">
                          FINISHED GOODS RECEIPT AND LABEL PRINTING
                      </p>
                  </div>
              </div>
          </div>
      ...
      <div className="px-4 sm:px-8 w-full">
        <div className="bg-white rounded-3xl shadow-lg border border-[#eaeaec] overflow-hidden flex flex-col min-h-[550px] animate-fadeIn p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <h4 className="font-black uppercase tracking-widest text-lg">Pending Receipts (ตรวจรับจริงและตรวจปล่อยสินค้า FG)</h4>
                <div className="flex items-center gap-2">
                    <div className="relative">
                        <Icons.Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                        <input 
                            type="text" 
                            placeholder="Scan or type Receipt ID, SO, SKU..." 
                            value={scanQuery}
            ...
            {isFormScannerOpen && (
              <BarcodeScanner 
                expectedType="inbound"
                title="Finished Goods Field Auto-Populator"
            ...
            {/* MULTI-FIELD FG GOODS RECEIPT FORM MODAL */}
            <DraggableModal
              ...
              title={
                <div className="flex items-center gap-2.5 text-left text-white">
                  ...
                  <div>
                    <h3 className="font-black text-[13px] uppercase tracking-widest text-[#eeeeee]">ตรวจรับสินค้าสำเร็จรูปใหม่ (FG Goods Receipt Entry)</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">WMS Digital FG Release & Print System</p>
                  </div>
                </div>
              }
            >
                ...
                <div className="mb-5 bg-[#3f809e]/10 border border-[#3f809e]/30 px-4 py-3 rounded-2xl flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11.5px] font-black text-slate-700">มีพาเลทการผลิตหรือรหัสบาร์โค้ดผลิตภัณฑ์? สแกนเพื่อกรอกข้อมูลรายละเอียดอัตโนมัติ!</p>
                    </div>
                  </div>
                  <button
                    type="button"
                ...
                <form onSubmit={handleAddReceiptSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">เลขที่ใบเสร็จรับสินค้า GR ID (ว่างเพื่อเจนอัตโนมัติ)</label>
                        <input
                          type="text"
                          placeholder="เช่น FG-IN-2606-101"
                          value={formGR.id}
                      ...
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">สายการผลิต / อ้างอิง Production Line (SO) *</label>
                        <div className="relative">
                          <input
                            type="text"
                            placeholder="เช่น PRD-LINE-A"
                            required
                            value={formGR.so}
                            onChange={(e) => setFormGR({ ...formGR, so: e.target.value })}
                          ...
                          <button
                            ...
                            title="สแกนเพื่อกรอกใบ SO / Production Line"
                          >
                            <Icons.Scan size={15} />
                          </button>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">รหัสสินค้าสำเร็จรูป SKU *</label>
                          <div className="relative">
                            <input
                              type="text"
                              placeholder="เช่น FG-8801"
                              required
                              value={formGR.sku}
                        ...
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">หน่วยบรรจุ (Unit)</label>
                          <select
                            value={formGR.unit}
                            onChange={(e) => setFormGR({ ...formGR, unit: e.target.value })}
                            className="w-full bg-white border border-slate-200 text-slate-800 text-xs font-bold rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#3f809e]"
                          >
                            <option value="BOX">BOX (กล่องพรีเมียม)</option>
                            <option value="PACKS">PACKS (แพ็ค 12)</option>
                            <option value="PALLET">PALLET (พาเลทใหญ่)</option>
                            <option value="PCS">PCS (ชิ้นแยกเดี่ยว)</option>
                          </select>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">ชื่อเรียกสินค้าสำเร็จรูป (Item Description) *</label>
                        <input
                          type="text"
                          placeholder="เช่น Nescafe Red Cup 380g"
                          required
                          value={formGR.name}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">จำนวนรับปล่อยจริง *</label>
                          <input
                            type="number"
                            placeholder="0"
                            required
                        ...
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">หมายเลขล็อตควบคุม *</label>
                          <div className="relative">
                            <input
                              type="text"
                              placeholder="เช่น L2026101"
                              required
                              value={formGR.lot}
                            ...
                            <button
                              ...
                              className="absolute right-2 top-1/2 -translate-y-1/2 text-[#3f809e] hover:text-[#212c46] p-1.5"
                              title="สแกนบาร์โค้ดเพื่อถอดรหัส Lot"
                            >
                              <Icons.Scan size={15} />
                            </button>
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">คงคลังก่อนรับ (Pre-Stock)</label>
                          <input
                            type="number"
                        ...
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">จุดเตือนสั่งผลิตซ้ำ (Min Safety)</label>
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">วันหมดอายุกำหนดพิเศษ (Expiry Date)</label>
                        <input
                          type="date"
                          value={formGR.exp}
                  ...
                  <div className="pt-4 border-t border-slate-100 flex justify-end gap-2">
                    ...
                    >
                      <Icons.CheckCircle2 size={14} /> อนุมัติบันทึกเข้าคลังและปล่อย Tag
                    </button>
                  </div>
                </form>
              </div>
                ...
                <table className="w-full text-left font-sans border-collapse">
                    <thead className="bg-[#133951] text-white sticky top-0 z-10 text-[12px]">
                        <tr>
                            ...
                            <th className="py-4 px-4 font-black uppercase tracking-widest border-b-2 border-[#ad2b10] text-[12px]">เอกสารรับสินค้าสำเร็จรูป (GR)</th>
                            <th className="py-4 px-4 font-black uppercase tracking-widest border-b-2 border-[#ad2b10] text-[12px]">รายละเอียดสินค้า</th>
                        ...
                        {mockReceipts.filter(r => !scanQuery || Object.values(r).join(' ').toLowerCase().includes(scanQuery.toLowerCase())).map(rec => (
                            <tr key={rec.id} className="hover:bg-[#f3f3f1]/60 transition-colors">
                                ...
                                <td className="py-2.5 px-4">
                                    ...
                                    <div className="text-[10px] text-slate-500 font-bold">{rec.so}</div>
                                </td>
                                <td className="py-2.5 px-4">
