...
export default function LawCalendar() {
  ...
  const [holidayForm, setHolidayForm] = useState({
    title: '', date: new Date().toISOString().split('T')[0], isRecurring: false
  });
  ...
  const [events, setEvents] = useState([
    ...
    { id: 'HOL-0101', date: '2026-01-01', title: 'วันขึ้นปีใหม่', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0303', date: '2026-03-03', title: 'วันมาฆบูชา', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0413', date: '2026-04-13', title: 'วันสงกรานต์', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0414', date: '2026-04-14', title: 'วันสงกรานต์', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0415', date: '2026-04-15', title: 'วันสงกรานต์', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0501', date: '2026-05-01', title: 'วันแรงงานแห่งชาติ', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0603', date: '2026-06-03', title: 'วันเฉลิมฯ พระราชินี', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0728', date: '2026-07-28', title: 'วันเฉลิมฯ รัชกาลที่ 10', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-0812', date: '2026-08-12', title: 'วันแม่แห่งชาติ', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-1013', date: '2026-10-13', title: 'วันหยุดชดเชย วันนวมินทรมหาราช', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-1023', date: '2026-10-23', title: 'วันปิยมหาราช', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-1205', date: '2026-12-05', title: 'วันพ่อแห่งชาติ', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
    { id: 'HOL-1231', date: '2026-12-31', title: 'วันสิ้นปี', time: 'All Day', type: 'Holiday', priority: 'Normal', status: 'Public Holiday', isRecurring: true, color: 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20' },
  ]);
  ...
  const processedEvents = useMemo(() => {
    return events.map((ev: any) => {
        if (ev.isRecurring) {
           ...
           return { ...ev, date: `${currentDate.getFullYear()}-${m}-${d}` };
        }
        return ev;
    });
  }, [events, currentDate]);
  ...
  const filteredEvents = useMemo(() => {
    return processedEvents.filter((ev: any) => {
  ...
  }, [processedEvents, searchQuery, currentDate, activeTab]);
  ...
  const handleSaveEvent = (e: any) => {
    ...
    const typeColors: any = {
      ...
      'Holiday': 'bg-[#932c2e]/10 text-[#932c2e] border-[#932c2e]/20'
    };
    ...
    // Simulate saving to Database/Google Sheets
    console.log("Mock saved to Google Sheets (GAS):", payload);
    const mockSaveNotification = document.createElement('div');
    mockSaveNotification.className = "fixed bottom-5 right-5 bg-green-500 text-white px-6 py-3 rounded-xl shadow-2xl z-[9999] font-bold text-sm animate-bounce";
    mockSaveNotification.innerText = "Saved to Google Sheets (Mock Database)";
    document.body.appendChild(mockSaveNotification);
    setTimeout(() => mockSaveNotification.remove(), 3000);
  };
  ...
  return (
    <div className="flex flex-1 w-full flex-col pb-5 animate-fadeIn bg-transparent space-y-6">
      ...
      <button onClick={() => setIsGuideOpen(true)} className="fixed right-0 top-[80px] py-8 px-1.5 fixed bg-white border border-[#eaeaec] border-r-0 text-[#212c46] rounded-l-xl shadow-md hover:bg-[#d96245] hover:text-white hover:border-[#d96245] transition-all duration-500 z-[100] flex flex-col items-center gap-4 group">
      ...
      <div className="px-4 sm:px-8 sm: w-full mt-[2px] pb-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-[#eaeaec]/60 overflow-hidden flex flex-col animate-fadeIn">
                <div className="px-8 py-4 bg-[#f8f9fa] flex flex-col md:flex-row justify-between items-center gap-4 border-b border-[#eaeaec]/60 shrink-0">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
                        ...
                        <button onClick={() => setIsHolidayModalOpen(true)} className="bg-white text-[#d96245] px-5 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-sm hover:bg-[#d96245] hover:text-white transition-all flex items-center gap-2 border border-[#eaeaec] hover:border-[#d96245] shrink-0">
                    ...
                    {activeTab === 'calendar' ? (
                        <div className="grid grid-cols-7 border-b border-[#eaeaec]">
                            ...
                            {calendarDays.map((d, i) => {
                                const dayEvents = processedEvents.filter((e: any) => e.date === d.dateStr);
                                return (
                        ...
                        <table className="w-full text-left font-sans border-collapse">
                            <thead className="bg-[#212c46] text-white sticky top-0 z-10 border-b-2 border-[#b58c4f]">
                                <tr>
                                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">รหัสอ้างอิง</th>
                                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">รายละเอียดเวลา</th>
                                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">รายละเอียดกิจกรรมทางกฎหมาย</th>
                                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center">หมวดหมู่</th>
                                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center">ระดับความสำคัญ</th>
                                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center">การจัดการ</th>
                                </tr>
                            </thead>
            ...
            <div className="bg-[#f8f9fa] rounded-3xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden relative border border-[#d96245]">
                <div className="bg-[#d96245] px-8 py-6 flex justify-between items-center shrink-0 border-b-4 border-[#ce870a] relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-5 relative z-10">
                      ...
                      <div>
                        <h3 className="text-xl font-black text-white uppercase tracking-widest leading-none">Add Holiday</h3>
                        <p className="text-[12px] font-bold text-white/80 uppercase tracking-widest mt-1.5 flex items-center gap-2">
                           <Palmtree size={12} className="text-white" /> Public Holiday Config
                        </p>
                      </div>
                    </div>
                    <button onClick={()=>setIsHolidayModalOpen(false)} className="text-white/80 hover:text-white transition-all bg-white/5 hover:bg-white/10 p-2.5 rounded-full active:scale-90 relative z-10"><X size={20} /></button>
                </div>
                
                <div className="p-8 overflow-y-auto custom-scrollbar">
                    <form id="holidayForm" onSubmit={(e) => {
                        e.preventDefault();
                        const newEvent = {
                            id: `HOL-${Date.now()}`,
                        ...
                        <div className="flex items-center gap-3 mt-4">
                           <input type="checkbox" id="isRecurring" checked={holidayForm.isRecurring} onChange={e=>setHolidayForm({...holidayForm, isRecurring: e.target.checked})} className="w-4 h-4 cursor-pointer accent-[#d96245]" />
                           <label htmlFor="isRecurring" className="text-[12px] font-black text-[#212c46] uppercase tracking-widest cursor-pointer mt-0.5">Repeat Every Year (ซ้ำทุกปี)</label>
                        </div>
                    </form>
                </div>
