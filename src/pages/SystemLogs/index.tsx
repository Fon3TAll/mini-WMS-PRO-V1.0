...
const generateMockLogs = () => {
    const logs = [];
    const modules = ['Authentication', 'User Permission', 'Item Master', 'Sales Order', 'Inventory', 'System Config', 'FG Inbound', 'FG Outbound', 'RM Inbound', 'RM Outbound'];
    const actions = ['LOGIN_SUCCESS', 'LOGIN_FAILED', 'DATA_EXPORT', 'RECORD_CREATED', 'RECORD_DELETED', 'PERMISSION_CHANGED', 'UNAUTHORIZED_ACCESS', 'INBOUND_RECEIVED', 'QA_INSPECTED', 'OUTBOUND_DISPATCHED', 'STATUS_UPDATED'];
    const users = ['SOMCHAI SALES', 'SUDA MARKETING', 'SMART LAW Developer', 'SARAH SUPPORT', 'UNKNOWN_USER', 'WAREHOUSE.ADMIN'];
...
export default function AccessLogs() {
    ...
    const uniqueUsersArray = Array.from(new Set(logs.map(l => l.user)));
    ...
    const filteredLogs = useMemo(() => {
        ...
        if (dateFilter) {
            res = res.filter(l => l.timestamp.startsWith(dateFilter));
        }
        if (userFilter) {
            res = res.filter(l => l.user === userFilter);
        }
        if (searchQuery) {
        ...
        return res;
    }, [logs, statusFilter, dateFilter, userFilter, searchQuery]);
                        ...
                        <div className="px-8 py-4 border-b border-[#eaeaec] bg-[#f8f9fa] flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
                            <div className="flex items-center gap-3 w-full md:w-auto">
                                <div className="relative">
                                    <button onClick={() => setFilterDropdownOpen(!filterDropdownOpen)} className="flex items-center gap-3 bg-[#f8f9fa] px-4 py-2.5 rounded-xl border border-[#eaeaec] shadow-sm hover:border-[#b7a159] hover:bg-white transition-all min-w-[150px]">
                                ...
                                <div className="relative">
                                    <input 
                                        ...
                                        className="bg-[#f8f9fa] px-4 py-2.5 rounded-xl border border-[#eaeaec] shadow-sm hover:border-[#b7a159] hover:bg-white transition-all text-[11px] font-black uppercase text-[#212c46] focus:outline-none"
                                    />
                                </div>
                                <div className="relative">
                                    <select 
                                        ...
                                        className="bg-[#f8f9fa] px-4 py-2.5 rounded-xl border border-[#eaeaec] shadow-sm hover:border-[#b7a159] hover:bg-white transition-all text-[11px] font-black uppercase text-[#212c46] focus:outline-none min-w-[150px] appearance-none"
                                    >
                                        <option value="">ALL USERS</option>
                                        {uniqueUsersArray.map((user: any) => (
                                ...
                                <div className="relative flex-1 md:w-64">
                                    ...
                                    <input type="text" value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} placeholder="Search IP or module..." className="w-full pl-10 pr-4 py-2.5 text-[12px] font-bold rounded-xl border border-[#eaeaec] focus:outline-none focus:border-[#b7a159] bg-[#f8f9fa] focus:bg-white shadow-sm text-[#212c46] transition-all" />
                                </div>
                            </div>
                        ...
                        <div className="overflow-x-auto custom-scrollbar bg-[#f8f9fa]">
                            <table className="w-full text-left font-sans border-collapse">
                                <thead className="bg-[#212c46] text-white sticky top-0 z-10 border-b-2 border-[#b7a159]">
                                    <tr>
                                        <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">วันที่ / เวลา</th>
                                        <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">ข้อมูลพนักงาน</th>
                                        <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">หมายเลข IP</th>
                                        <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap">โมดูลย่อย & คำสั่งที่ดำเนินการ</th>
                                        <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center whitespace-nowrap">สถานะ</th>
                                        <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center whitespace-nowrap">การจัดการ</th>
                                    </tr>
                                </thead>
