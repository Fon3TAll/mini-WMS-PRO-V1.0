...
import WorkspaceSyncConfig from '../../components/WorkspaceSyncConfig';
import { 
  Settings2, 
  Building2, 
  Layers, 
...
const TABS = [
  ...
  { id: 'googleSheets', label: 'GOOGLE SHEETS', icon: 'Database', title: 'GOOGLE SHEETS SYNC', desc: 'Manage Google Sheets integrations and formatting.' }
];
                    ...
                    {TABS.map(tab => (
                        ...
                        >
                            ...
                            <div className="flex-1 text-left overflow-hidden">
                                ...
                                <p className={`text-[11px] font-bold uppercase tracking-widest mt-0.5 truncate ${activeTab === tab.id ? 'text-[#b7a159]' : 'text-[#7a8b95]'}`}>{(data[tab.id]?.length || 0)} Items</p>
                            </div>
                        ...
                        <table className="w-full text-left font-sans border-collapse">
                            <thead className="bg-[#212c46] border-b-2 border-[#b7a159] text-white uppercase tracking-widest text-[12px] font-black sticky top-0 z-10">
                                <tr>
                                    {activeTab === 'idFormats' ? (
                                        <>
                                            <th className="py-4 px-6 whitespace-nowrap text-[12px]">โมดูลการใช้งาน (Pages)</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">คำขึ้นต้นหลัก (Prefix)</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">รูปแบบรหัสตัวเลข (Format)</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">กฎและเงื่อนไขการรันเลข</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">การจัดการ</th>
                                        </>
                                    ) : activeTab === 'pdfTemplates' ? (
                                        <>
                                            <th className="py-4 px-6 whitespace-nowrap text-[12px]">แม่แบบเอกสารพิมพ์ PDF</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">แผนกปฏิบัติงานหลัก</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">รหัสเอกสารอ้างอิง</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">ครั้งที่ปรับปรุง (Revision)</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">การจัดการ</th>
                                        </>
                                    ) : (
                                        <>
                                            <th className="py-4 px-6 whitespace-nowrap text-[12px]">ชื่อเรียกที่ระบุตัวตน</th>
                                            {activeTab === 'departments' && <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">รหัสระบบ (Sys Code)</th>}
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">สถานะเปิดใช้งาน</th>
                                            <th className="py-4 px-6 text-center whitespace-nowrap text-[12px]">การจัดการ</th>
                                        </>
                                    )}
                                </tr>
